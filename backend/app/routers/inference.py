...
async def scan_image(file: UploadFile = File(...)):
    ...
    total_acc = len(deduped)
    high = severity_counts["high"]
    if high > 0:
        summary = f"Found {total_acc} accessibility feature(s) — {high} critical issue(s) require attention."
    elif total_acc > 0:
        summary = f"Found {total_acc} accessibility feature(s). No critical barriers detected."
    else:
        summary = "No known accessibility features detected in this image."
    ...
    return {
        ...
        "accessibility_features": total_acc,
